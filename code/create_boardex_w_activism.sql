...
WITH committees AS (
  ...
  ages AS (
    SELECT DISTINCT a.boardid, a.directorid, a.annual_report_date,
                    COALESCE(dob, '2015-07-01'::DATE - age*365) AS dob,
                    (a.annual_report_date - COALESCE(dob, '2015-07-01'::DATE - age*365))/365.25 AS age
  ...
  AND a.annual_report_date=e.annual_report_date
  LEFT JOIN ages AS f
  ON a.boardid=f.boardid AND a.directorid=f.directorid
  AND a.annual_report_date=f.annual_report_date
  ...
  ON a.cusip=b.ncusip),
    ...
    boardex_activism_match AS (
        ...
        ON b.permno=c.permno AND
            a.annual_report_date BETWEEN c.eff_announce_date AND c.end_date + interval '128 days'
    ...
    SELECT DISTINCT a.permno, a.annual_report_date, a.boardid,
