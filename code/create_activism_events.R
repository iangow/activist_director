...
dbGetQuery(pg, "DROP TABLE IF EXISTS activism_events_new")
...
matched <-
    ...
    mutate(affiliated = case_when(
        ...
        big_investment = case_when(
	         ...
	         TRUE ~ category)) %>%
    compute(name = "activism_events_new", temporary = FALSE)

sql <- paste0("

    ALTER TABLE activism_events_new OWNER TO activism;

    COMMENT ON TABLE activism_events_new IS
