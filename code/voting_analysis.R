...
rs <- dbGetQuery(pg, "
    ...
    mgmtrec, base, vote_pct, votes_cast --, issrec
    FROM director_votes
...
vote.data <- dbGetQuery(pg, "
    WITH activist_director_matches AS (
        SELECT DISTINCT a.permno, a.last_name, a.first_name, a.meetingdate, a.vote_pct, a.meetingdate_p1, a.vote_pct_p1, a.meetingdate_p2, a.vote_pct_p2,
    ...
    activist_director_years AS (
        ...
        AND meetingdate > appointment_date
    ...
    first_election_dates AS (
        ...
        FROM activist_director_matches
        WHERE activist_director AND vote_pct IS NOT NULL
        AND meetingdate BETWEEN appointment_date + INTERVAL '1 day' AND appointment_date + INTERVAL '3 years'
