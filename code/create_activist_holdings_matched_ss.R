...
holding_data <- dbGetQuery(pg, "
  ...
  partitions AS (
      ...
      --WHERE '3G Capital Management, Inc.' = any(activist_names)
...
exit_quarter_extra AS (
      ...
      SELECT DISTINCT a.activist_names, a.cusip, COALESCE(b.permno,c.permno) AS permno, datadate,
      ...
      ON a.cusip=c.ncusip
      --WHERE a.cusip IN ('90403T20', '90403T10')
      ...
      LEFT JOIN activist_director.financials AS c
