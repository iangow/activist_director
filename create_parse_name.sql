-- Functions and data structures for parsed names
DROP TYPE IF EXISTS activist_director.parsed_name CASCADE;

CREATE TYPE activist_director.parsed_name AS
   (prefix text,
   first_name text,
    middle_initial text,
    last_name text,
...
ALTER TYPE activist_director.parsed_name OWNER TO activism;

CREATE OR REPLACE FUNCTION activist_director.parse_name(text)
  RETURNS activist_director.parsed_name AS
$BODY$
...
ALTER FUNCTION activist_director.parse_name(text) OWNER TO activism;
