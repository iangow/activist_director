...
Consistent with prior research, we find significant risk-adjusted returns around the announcement of activism, with size-adjusted returns from $-20$ to $+20$ trading days around the announcement ranging from $5\%$ to $14\%$.
...
Looking beyond positive announcement-period returns, operating performance seems to improve, with return on assets increasing by more than $1-2\%$ over the five years after activism.
