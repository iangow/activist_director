...
Our sample of \Sexpr{num_events} activism events comprises all activism events targeted at US companies from 2004 to 2015.\footnote{We additionally require that the target firm is matched to CRSP, is not an investment trust or mutual fund, and that the event is not a control contests involving another corporation.}
...
Activist-affiliated directors (i.e., employees or principals of members of the activist group) are about 12 years younger than other new directors and much less frequently female.
...
Consistent with prior research, we find significant risk-adjusted returns around the announcement of activism, with size-adjusted returns from $-20$ to $+20$ trading days around the announcement ranging from $4\%$ to $5\%$.
