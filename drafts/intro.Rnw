...
Third, board positions also come with fiduciary responsibilities to all shareholders and may limit activists' ability to trade in the target stock when possessing inside information.
...
How do these effects compare to those of other forms of activism?
...
These holding periods suggest that activists with board representation may be considered ``long-term'' investors.\footnote{As discussed in Section \ref{sect:holding}, pension funds have a typical duration of 2 years, and investor relation professionals only consider a horizon of more than 2.8 years to be ``long-term.''}

With regard to the next set of questions, consistent with prior research, our findings show that activists tend to target firms with relatively poor recent stock performance and those that are considered ``value'' firms, i.e., those with low market-to-book ratios.
Conditional on being targeted by activists, activists are more likely to demand or acquire board representation if the firm has higher levels of institutional ownership, a smaller market capitalization, worse stock market performance, and, in particular, lower dividend payout levels.
...
We find that operating performance improves after activism, with a return on assets increase of more than $1.7$ percentage points over a five-year period after the announcement.
We also observe differential effects when \emph{activist-affiliated} directors are appointed, with an additional $1.6\%$ through $3.2\%$ percentage-point increase in return on assets two to five years after the announcement, respectively.
In terms of market returns, consistent with prior research, we find significant risk-adjusted returns around the announcement of activism, with size-adjusted returns from $-1$ to $+1$ trading days around the announcement ranging from $3.0\%$ to $3.9\%$.
...
The paper closest to this one is \citet{Bebchuk:2017}, who examined settlement agreements between activists and target firms.
...
However, our study differs in several key ways.
First, our sample period is more recent, allowing us to capture the most up-to-date trends and insights.
Second, our study includes a broader range of activist directors, including those appointed outside of settlement agreements.
For instance, our sample covers 1,623 activist directors compared to 634 activist directors in \citet{Bebchuk:2017}.
This allows us to offer a more comprehensive and in-depth analysis of the role of activist directors in the broader context of hedge fund activism.
Third, we delve more deeply into the characteristics of activist directors, including their demographics and skill sets.
...
In November 2021, the SEC voted to adopt new rules requiring the use of a single proxy card for director elections, which are set to take effect in August 2022.
These changes are expected to make it easier for activists to nominate directors, which could lead to an increase in the number of activist directors in the future.
