...
\emph{Acquisition} & Indicator for whether the firm completed any acquisitions in the two years after a given fiscal year \\
\emph{Divestiture} & Indicator for whether the firm divested significant assets in the two years after a given fiscal year \\
\emph{Acquired} & Indicator for whether the firm was acquired in the two years after a given fiscal year \\
\emph{CEO exit\textsubscript{$(t,t+k)$}} & Change in CEO between the end of year $t$ and the end of year $t + k$ \\
\emph{CEO comp\textsubscript{$t$}} & Total CEO compensation in year $t$ \\
\emph{Payout\textsubscript{$t$}} & Ratio of total dividends and share repurchases to EBITDA for the two years after year $t$ \\
\emph{Leverage\textsubscript{$t$}} & Ratio of book value of debt to the sum of the book value of debt and equity in year $t$ \\
\emph{Capex\textsubscript{$t$}} & Ratio of total capital expenditure to lagged total assets in year $t$ \\
\emph{R\&D\textsubscript{$t$}} & Ratio of total R\&D expenditure to lagged total assets in year $t$ \\
