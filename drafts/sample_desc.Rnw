...
For each activism event, we measure the effective announcement date using various sources, including SharkWatch and relevant SEC filings, as the earliest date on which the activism became clearly known to the market.
Because many of our empirical analyses use data reported in fiscal periods, we denote the fiscal year in which the effective announcement date occurs as year $t + 1$ and consider the previous fiscal year (year $t$) as the year prior to activism.
Year $t$ serves as reference year in many of our empirical analyses.
