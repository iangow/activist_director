...
dbExecute(pg, "SET search_path TO activist_director, public")
...
ad_years <-
    ...
    inner_join(activism_events,
               by = c("campaign_id", "permno", "dissident_group", "eff_announce_date")) %>%
    mutate(still_active = as.integer(is.na(retirement_date)),
           ...
           tenure_censored = coalesce(retirement_date, '2017-12-31') - appointment_date,
           year = date_part('year', eff_announce_date)) %>%
    filter(!is.na(appointment_date), !is.na(permno))
...
<<table_outcomes, child='tables/table_outcomes.Rnw'>>=
@
