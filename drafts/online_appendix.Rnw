...
Smaller firms are also more likely to be targeted for board-related activism.
...
We also find negative associations between R\&D spending and activist directors (coefs. -0.007 and -0.009 for Affiliated director and Unaffiliated director, respectively).
The differences between these coefficients and that for {Other activism} are weakly significant (p $<$ 0.10) for both categories.
