...
fix_activist_director <- function(x) {
    case_when(x == "affiliated" ~ "activist_director",
              x %in% c("unaffiliated", "activist_director") ~ "activist_director",
              x %in% c("activist_demand", "activism") ~ "z_other_activism",
              TRUE ~ "_none")
}
...
Regressions include industry and year fixed effects and controls are as described in Table \ref{tbl:demand}; coefficients omitted for space.
...
rhs.1 <- paste0(paste(rhs.1, collapse = " + "))
...
rhs.2 <- paste0(paste(rhs.2, collapse = " + "))
...
\textit{Payout} is measured as the ratio of total dividends and share repurchases to EBITDA for the two years after year $t$.
\textit{R\&D} is measured as the ratio of total R\&D expenditure for two years after announcement of activism to the lagged book value of total assets.
Regressions include industry and year fixed effects and controls are as described in Table \label{tbl:demand}; coefficients omitted for space.
Values in parentheses are standard errors clustered by firm.
*** (**, *) indicates significance at the 1\% (5\%, 10\%) level.
\newline}
% \end{flushleft}
