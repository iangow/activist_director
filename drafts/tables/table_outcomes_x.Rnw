...
All regressions include industry and year fixed effects and controls, measured as of the previous fiscal year-end $t$, as described in Section \ref{sec:other_data}; coefficients omitted for space.
