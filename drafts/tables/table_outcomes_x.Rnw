...
appointment_dates %>%
    ...
    mutate(delist_pre = delist && between(dlstdt, eff_announce_date, appointment_date),
           delist_post = delist && between(dlstdt, appointment_date,
                                           sql("eff_announce_date + interval '3 years'"))) %>%
    summarize(delist_post = sum(as.integer(delist_post)),
              delist_pre = sum(as.integer(delist_pre)))
