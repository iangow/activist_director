...
source("~/activist_director/drafts/tables/outcome_functions_x.R")
@
...
reg.data.ceo <-
    ...
    mutate(affiliated = if_else(affiliated=="affiliated", "affiliated",
                                if_else(affiliated %in% c("unaffiliated", "activist_director"),
                                        "unaffiliated",
                                        if_else(affiliated %in% c("activist_demand", "activism"),
                                                "z_other_activism",
                                                "_none")))) %>%
    distinct() %>%
    left_join(activist_demands) %>%
...
\begin{table}[htb]
\caption{Other outcomes and demand}
\label{tbl:other_x}
\begin{minipage}[t]{\columnwidth}
% \begin{flushleft}
{\small
...
Controls are as described in Table \ref{tbl:div_acq_x}; coefficients omitted for space.
Values in parentheses are standard errors clustered by firm.
*** (**, *) indicates significance at the 1\% (5\%, 10\%) level.
}
% \end{flushleft}
...
# CEO Turnover
rhs.1 <- "activist_director*remove_officer_demand year sic2
        analyst inst size_return log_at mv btm leverage payout roa sale_growth
        num_directors outside_percent staggered_board"
...
rhs.2 <- "activist_director*leverage_demand year sic2
        analyst inst size_return log_at mv btm leverage payout roa sale_growth
        num_directors outside_percent staggered_board"

rhs.3 <- "activist_director*payout_demand year sic2
        analyst inst size_return log_at mv btm leverage payout roa sale_growth
        num_directors outside_percent staggered_board"

rhs.4 <- "activist_director*other_gov_demand year sic2
        analyst inst size_return log_at mv btm leverage payout roa sale_growth
        num_directors outside_percent staggered_board"
...
t.pb[[4]] <- fit.model(data=reg.data,
                       lhs="rnd_cum", rhs=rhs.4, cluster1="permno")
...
stargazer(getFitted(t.pb),
          ...
          covariate.labels = c("Activist director",
                               ...
                               "Activist director x Remove officer demand",
                               "Leverage demand",
                               "Activist director x Leverage demand",
                               "Payout demand",
                               "Activist director x Payout demand",
                               "Other governance demand",
                               "Activist director x Other governance demand"),
          se=getSEs(t.pb),
          p=getPs(t.pb),
          float=FALSE, align=TRUE, font.size="small",
          keep.stat=c("n"), no.space=TRUE,
