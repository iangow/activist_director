...
\caption{Career consequences for activist directors}
    ...
    {\small \newline
Panel A presents results from OLS regressions where the dependent variable is $\textit{Departure}(t, t+2)$, an indicator for the director leaving the board of the firm by year \textit{t}+2 (i.e., the year after the activism event, if any).
Column 1 presents OLS results for all directors. Columns 2 and 3 focus on first-year directors (just appointed).
In Columns 4 and 5, we focus on the sample of firms targeted by activists.
...
# Tabulate results
library(stargazer)
stargazer(fm.t1.c1, fm.t1.c2, fm.t1.c4, fm.t1.c5, fm.t1.c6,
          omit = c("Constant", "(factor.*)"),
          column.labels =  c("All dir.", "New dir.", "New firm", "Activism", "Act. demand"),
          dep.var.caption = "",
          dep.var.labels.include = FALSE, # "$\\textit{Other Boards}_{t+2}$",
          float=FALSE, align=TRUE, font.size="small",
          keep.stat=c("n"), no.space=TRUE,
          covariate.labels = c("Activist director", "Outsider", "Age", "Female",
                               "Comp. Comm.", "Audit. Comm.", "Analyst", "Inst", "Size-adj. Ret.",
                               "Market Value", "Book-to-Market", "Leverage", "Dividend", "ROA", "Sales Growth"),
          se=list(fm.t1.c1.se[,2], fm.t1.c2.se[,2], fm.t1.c4.se[,2], fm.t1.c5.se[,2], fm.t1.c6.se[,2]))
@
\end{tabularx}
\end{center}
...
\caption{Career consequences for activist directors}
    ...
    {\small \newline
Panel B presents results from OLS regressions where the dependent variable is $\textit{Departure}(t, t+2)$, an indicator for the director leaving the board of the firm by year $t+2$ (i.e., the year after the activism event, if any).
Column 1 presents OLS results for all directors.
Columns 2 and 3 focus on first-year directors (just appointed).
In Columns 4 and 5, we focus on the sample of firms targeted by activists.
All variables are defined in Appendix A.
All regressions include year fixed-effects and robust standard errors (in parentheses) clustered at the firm level.
        \newline}
...
library(stargazer)
stargazer(fm.t2.c1, fm.t2.c2, fm.t2.c4, fm.t2.c5, fm.t2.c6,
          omit = c("Constant", "(factor.*)"),
          column.labels =  c("All dir.", "New dir.", "New firm", "Activism", "Act. demand"),
          dep.var.caption = "",
          dep.var.labels.include = FALSE, # "$\\textit{Other Boards}_{t+2}$",
          float=FALSE, align=TRUE, font.size="small",
          keep.stat=c("n"), no.space=TRUE,
          covariate.labels = c("Affiliated director", "Unaffiliated director", "Outsider", "Age", "Female",
                               "Comp. Comm.", "Audit. Comm.", "Analyst", "Inst", "Size-adj. Ret.",
                               "Market Value", "Book-to-Market", "Leverage", "Dividend", "ROA", "Sales Growth"),
          se=list(fm.t2.c1.se[,2], fm.t2.c2.se[,2], fm.t2.c4.se[,2], fm.t2.c5.se[,2], fm.t2.c6.se[,2]))
@
\end{tabularx}
\end{center}
\addtocounter{table}{-1}
...
\caption{Career consequences for activist directors}
    ...
    {\small \newline
The table presents results from OLS regressions where the dependent variable is $\textit{Other Boards}_{t+2}$, the number of directorships a director has with other firms other in year \textit{t}+2 (i.e., the year after the activism event, if any).
Columns 1 presents results for all directors; Columns 2 and 3 present results for first-year directors and first-year independent directors, respectively; Columns 4 and 5 present results for firms with activism or activist demand for board seats, respectively.
All variables are defined in Appendix A.
All regressions include year fixed-effects and robust standard errors (in parentheses) clustered at the firm level.
\newline}
    \end{minipage}
...
# Produce Excel file with results for Table
library(stargazer)
stargazer(fm.t3.c1, fm.t3.c2, fm.t3.c4, fm.t3.c5, fm.t3.c6,
          omit = c("Constant", "(factor.*)"),
          column.labels =  c("All dir.", "New dir.", "New firm", "Activism", "Act. demand"),
          dep.var.caption = "",
          dep.var.labels.include = FALSE, # "$\\textit{Other Boards}_{t+2}$",
          float=FALSE, align=TRUE, font.size="small",
          keep.stat=c("n"), no.space=TRUE,
          covariate.labels = c("Other Boards", "Activist Director", "Outsider", "Age", "Female",
                               "Comp. Comm.", "Audit. Comm.", "Analyst", "Inst", "Size-adj. Ret.",
                               "Market Value", "Book-to-Market", "Leverage", "Dividend", "ROA", "Sales Growth"),
          se=list(fm.t3.c1.se[ ,2], fm.t3.c2.se[ ,2],
                  fm.t3.c4.se[ ,2], fm.t3.c5.se[ ,2],
                  fm.t3.c6.se[ ,2]))
@
\end{tabularx}
\end{center}
...
Columns 1 presents results for all directors;
Columns 2 and 3 present results for first-year directors and first-year independent directors, respectively;
Columns 4 and 5 present results for firms with activism or activist demand of board seat, respectively.
All variables are defined in Appendix A.
All regressions include year fixed-effects and robust standard errors (in parentheses) clustered at the firm level.
        \newline}
    \end{minipage}
...
# Produce Excel file with results for Table
library(stargazer)
stargazer(fm.t4.c1, fm.t4.c2, fm.t4.c4, fm.t4.c5, fm.t4.c6,
          omit = c("Constant", "(factor.*)"),
          column.labels =  c("All dir.", "New dir.", "New firm", "Activism", "Act. demand"),
          dep.var.caption = "",
          dep.var.labels.include = FALSE, # "$\\textit{Other Boards}_{t+2}$",
          float=FALSE, align=TRUE, font.size="small",
          keep.stat=c("n"), no.space=TRUE,
          covariate.labels = c("Other Boards", "Affiliated director", "Unaffiliated director", "Outsider", "Age", "Female",
                               "Comp. Comm.", "Audit. Comm.", "Analyst", "Inst", "Size-adj. Ret.",
                               "Market Value", "Book-to-Market", "Leverage", "Dividend", "ROA", "Sales Growth"),
          se=list(fm.t4.c1.se[,2], fm.t4.c2.se[,2], fm.t4.c4.se[,2], fm.t4.c5.se[,2], fm.t4.c6.se[,2]))
@
\end{tabularx}
\end{center}
\addtocounter{table}{-1}
