...
\caption{Career consequences for activist directors}
    ...
    {\small \newline
Panel B presents results from OLS regressions where the dependent variable is Departure($t$, $t+2$), i.e., an indicator for the director leaving the board of the firm by year $t+2$ (i.e., the year after the activism event, if any).
