...
outcome_controls_mod <-
    ...
    select(permno, eff_announce_date, first_appointment_date, matches("^(datadate|default_|firm_exists_)"))
...
divestiture_count <-
    ...
    mutate(divest_p2 = if_else(!is.na(first_appointment_date), between(announcedate, first_appointment_date, sql("first_appointment_date + interval '2 years'")),
                                      if_else(!is.na(eff_announce_date), between(announcedate, eff_announce_date, sql("eff_announce_date + interval '2 years'")),
                                                     between(announcedate, datadate, datadate_p2))),
           divest_p3 = if_else(!is.na(first_appointment_date),between(announcedate, first_appointment_date, sql("first_appointment_date + interval '3 years'")),
                                      if_else(!is.na(eff_announce_date), between(announcedate, eff_announce_date, sql("eff_announce_date + interval '3 years'")),
                                                     between(announcedate, datadate, datadate_p3)))) %>%
...
acquisition_count <-
    ...
    mutate(acquisition_p2 = if_else(!is.na(first_appointment_date), between(announcedate, first_appointment_date, sql("first_appointment_date + interval '2 years'")),
                                      if_else(!is.na(eff_announce_date), between(announcedate, eff_announce_date, sql("eff_announce_date + interval '2 years'")),
                                                     between(announcedate, datadate, datadate_p2))),
           acquisition_p3 = if_else(!is.na(first_appointment_date),between(announcedate, first_appointment_date, sql("first_appointment_date + interval '3 years'")),
                                      if_else(!is.na(eff_announce_date), between(announcedate, eff_announce_date, sql("eff_announce_date + interval '3 years'")),
                                                     between(announcedate, datadate, datadate_p3)))) %>%
    group_by(permno, datadate) %>%
...
spinoff_count <-
    ...
    mutate(spinoff_p2 = if_else(!is.na(first_appointment_date), between(announcedate, first_appointment_date, sql("first_appointment_date + interval '2 years'")),
                                      if_else(!is.na(eff_announce_date), between(announcedate, eff_announce_date, sql("eff_announce_date + interval '2 years'")),
                                                     between(announcedate, datadate, datadate_p2))),
           spinoff_p3 = if_else(!is.na(first_appointment_date),between(announcedate, first_appointment_date, sql("first_appointment_date + interval '3 years'")),
                                      if_else(!is.na(eff_announce_date), between(announcedate, eff_announce_date, sql("eff_announce_date + interval '3 years'")),
                                                     between(announcedate, datadate, datadate_p3)))) %>%
    group_by(permno, datadate) %>%
...
\textit{Acquisition} (\textit{Divestiture}, or \textit{Acquired}) indicates the firm completed acquisitions (divestitures and spinoffs, or delisting (CRSP delisting code $\textrm{dlstcd} \in [200, 399]$) in years $t+1$ and $t+2$, where activism was announced in year $t+1$, or during the two years following the announcement of activism for the activism sample and the two years following the appointment of an activist director for the activist director sample.
