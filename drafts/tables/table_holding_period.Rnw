...
survival_data <- dbGetQuery(pg, "
    ...
    WITH activist_exit AS (
        SELECT activist_name, permno, first_date, announce_date, bool_or(exit IS TRUE) AS exit
        FROM activist_director.activist_holdings_matched
        GROUP BY activist_name, permno, first_date, announce_date)
    ...
    USING (activist_name, permno, first_date, announce_date)
