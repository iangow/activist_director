...
survival_data <- dbGetQuery(pg, "
    ...
    WITH activist_exit AS (
        SELECT activist_name, permno, entry_date,
    ...
    SELECT DISTINCT activist_name, permno, first_date, entry_date,
        exit_date, last_date, first_appointment_date,
        eff_announce_date, activism, activist_demand, activist_director,
        category, affiliated, big_investment, b.exit
    FROM activist_director.activist_holdings
...
panel2 <- formatC(panel2, digits=0, format="f")
