...
<<quantreg_data_step, include=FALSE, message=FALSE, warning=FALSE>>=
library(survival)
library(quantreg)
library(RPostgreSQL)
...
survival_data <- dbGetQuery(pg, "
  ...
  SELECT DISTINCT activist_name, permno, first_date, entry_date, exit_date, last_date,
    announce_date, first_appointment_date, eff_announce_date,
	activism, activist_demand, activist_director, category, big_investment,
	bool_or(exit IS TRUE) AS exit
  ...
  GROUP BY
	activist_name, permno, first_date, entry_date, exit_date, last_date,
	announce_date, first_appointment_date, eff_announce_date,
	activism, activist_demand, activist_director, category, big_investment
...
<<make_panels, dependson="quantreg_data_step", include=FALSE>>=
...
@
\begin{table}[htb]  \label{tbl:holdings}
