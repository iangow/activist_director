...
quantreg1 <- crq(Surv(as.numeric(exit_date - entry_date), exit, type='right') ~ affiliated,
                 ...
                 na.action="na.omit",
                 method="Portnoy")
summary1 <- summary(quantreg1, taus = 0.5)
...
quantreg2 <- crq(Surv(as.numeric(exit_date - eff_announce_date), exit, type='right') ~ affiliated,
                 ...
                 na.action="na.omit",
                 method="Portnoy")
summary2 <- summary(quantreg2, taus = 0.5)
...
quantreg3 <- crq(Surv(as.numeric(exit_date - first_appointment_date), exit, type='right') ~ affiliated - 1,
                 ...
                 na.action = "na.omit",
                 method = "Portnoy")
summary3 <- summary(quantreg3, taus = 0.5)
@
...
\small
\tabularnewline
This table presents the results of censored median regression analysis of the holding period (days) on the activism category.
