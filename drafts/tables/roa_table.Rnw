...
% \begin{flushleft}
{\small
This table presents estimates of the impact of activism on return on assets over five years after the announcement of activism.
The empirical approach follows that of \citet{Bebchuk:2013wi}.
We regress the return on assets on an indicator for activism events ($\textit{Activism}_s$), where $s$ ranges from $t - 3$ to $t + 5$ and year $t + 1$ is defined as the year in which activism was announced.
We estimate three models: (A), (B), and (C).
