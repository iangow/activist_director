...
summ.firms <- dbGetQuery(pg, "
    ...
    SELECT year,
        count(*) AS num_events,
        sum((category='activism')::int) AS nba,
        sum((category='activist_demand')::int) AS board_demand,
        sum((category='activist_director')::int) AS activist_director
    FROM raw_data
    GROUP BY year
    ORDER BY year")
...
summ.dirs <- dbGetQuery(pg, "
    WITH ad_years AS (
        SELECT DISTINCT a.*, b.elected,
            ...
            COALESCE(retirement_date, '2017-12-31')-appointment_date AS tenure_censored,
            extract(year FROM a.eff_announce_date) AS year
        ...
        WHERE appointment_date IS NOT NULL AND b.permno IS NOT NULL AND independent IS NOT NULL)

    SELECT year,
        COUNT(*) AS activist_director,
        sum((independent IS FALSE)::int) AS affiliated,
        sum((independent IS TRUE)::int) AS unaffiliated,
