...
ad_years <-
    activism_events %>%
    select(permno, dissident_group, eff_announce_date, elected) %>%
    distinct() %>%
    inner_join(activist_directors, by = c("permno", "dissident_group", "eff_announce_date")) %>%
    filter(!is.na(appointment_date), !is.na(permno), !is.na(independent)) %>%
    mutate(still_active = is.na(retirement_date),
            left_board = !is.na(retirement_date),
