...
%  \begin{flushleft}
{\small
This table presents the number of activism events by year.
\textit{Activism} refers to activism events in any of the following three mutually exclusive categories.
\textit{Non-board activism} refers to non-board-related activism events.
...
\textit{Affiliated} (\textit{Unaffiliated}) indicates activist directors who are
  (are not) employees or principals of a member of the activist group.
\textit{Elected} indicates that the director was elected by shareholders through a shareholder vote.
\textit{Settled} indicates that the director was appointed to the board without a shareholder vote.
\newline}
%\end{flushleft}
\end{minipage}
\begin{tabularx}{\linewidth}{l*{8}{Y}}
