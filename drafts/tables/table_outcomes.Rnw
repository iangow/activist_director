...
sql <- paste0(readLines("~/activist_director/drafts/tables/outcome_div_acq.sql"), collapse="\n")
...
pg <- dbConnect(PostgreSQL())
rs <- dbGetQuery(pg, "SET work_mem='5GB'")
sql <- paste0(readLines("~/activist_director/drafts/tables/outcome_ceo.sql"), collapse="\n")
reg.data <- dbGetQuery(pg, sql)
rs <- dbDisconnect(pg)
...
pg <- dbConnect(PostgreSQL())
rs <- dbGetQuery(pg, "SET work_mem='5GB'")
sql <- paste0(readLines("~/activist_director/drafts/tables/other_outcomes.sql"), collapse="\n")
reg.data <- dbGetQuery(pg, sql)
rs <- dbDisconnect(pg)

# Winsorize changes
lhs.vars <- c("cash", "leverage", "payout", "capex_cum", "rnd_cum", "adv_cum")
winsor.vars <- unique(c(unlist(strsplit(controls, "\\s+")), lhs.vars, paste0(lhs.vars, "_p2"), paste0(lhs.vars, "_p3")))
