...
{\small
This table presents regressions of outcome variables on firm-year level activism indicators.
...
Regressions include industry and year fixed effects and the following controls (measured for the fiscal year-end of year $t$):
...
% \begin{flushleft}
{\small
This table presents regressions of variables on firm-year level activism categorical variables.
...
Controls are as described in Section 3.5; coefficients omitted for space.
Values in parentheses are standard errors clustered by firm.
*** (**, *) indicates significance at the 1\% (5\%, 10\%) level.
}
% \end{flushleft}
...
Controls are described in Section 3.5; coefficients omitted for space.
