...
reg.data <-
    ...
    mutate(category_activist_director = if_else(!is.na(category_activist_director), category_activist_director, "_none")) %>%
    as.data.frame()
...
row.labels <- c("Activist director","Non-Activist director")
...
row.labels <- c("Activist director", "Non-Activist director",
                "$\\textrm{Dep. var.}_{t}$", "$\\textrm{lagged.var}$")
names(summ) <- col.labels
