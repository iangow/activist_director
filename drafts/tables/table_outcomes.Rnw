...
divestiture_count <-
    ...
    summarize(num_divestiture_p2 = sum(as.integer(if_else(firm_exists_p2,
                                                          coalesce(divest_p2, FALSE), NA))),
              num_divestiture_p3 = sum(as.integer(if_else(firm_exists_p2,
                                                          coalesce(divest_p3, FALSE), NA)))) %>%
    ungroup() %>%
...
acquisition_count <-
    ...
    group_by(permno, datadate) %>%
    summarize(num_acquisition_p2 = sum(as.integer(if_else(firm_exists_p2,
                                                          coalesce(acquisition_p2, FALSE), NA))),
              num_acquisition_p3 = sum(as.integer(if_else(firm_exists_p2,
                                                          coalesce(acquisition_p3, FALSE), NA)))) %>%
    ungroup() %>%
...
spinoff_count <-
    ...
    group_by(permno, datadate) %>%
    summarize(num_spinoff_p2 = sum(as.integer(if_else(firm_exists_p2,
                                                      coalesce(spinoff_p2, FALSE), NA))),
              num_spinoff_p3 = sum(as.integer(if_else(firm_exists_p3,
                                                      coalesce(spinoff_p3, FALSE), NA)))) %>%
    ungroup() %>%
    compute()
...
reg.data <-
    ...
    mutate(activism = if_else(category=="_none", FALSE, TRUE)) %>%
    as.data.frame()
...
t6.pa <- lapply(lhs.vars, get.model.2, data=reg.data, use.controls=TRUE)
...
# Get data from PostgreSQL
pg <- dbConnect(PostgreSQL())
reg.data <- dbGetQuery(pg, "SELECT * FROM activist_director.ceo_outcomes")
rs <- dbDisconnect(pg)
...
\textit{CEO exit} indicates a change in CEO between the end of year $t$ and the end of year $t+2$.
% TODO: Is it end of t or end of t+1?
$\textit{CEO comp}_{t+2}$ is log of total CEO compensation in year $t+2$.
...
t6.pb <- lapply(lhs.vars, get.model.2, data=reg.data, include.lag=TRUE, use.controls=TRUE)
...
reg.data <- winsorizeDF(reg.data, winsor.vars)
reg.data$activism <- ifelse(reg.data$category=="_none",FALSE,TRUE)
@
...
t6.pd <- lapply(lhs.vars, get.model.2, data=reg.data, include.lag=TRUE, use.controls=TRUE)
row.labels <- c("Non-board activism", "Board demand", "Activist director",
                "$\\textrm{Dep. var.}_{t}$")
...
t6.pf <- lapply(lhs.vars, get.model.2, data=reg.data, include.lag=TRUE, use.controls=TRUE)
