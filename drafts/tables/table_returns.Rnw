...
merged <- collect(event_returns)

rs <- dbDisconnect(pg)

merged$affiliated <- as.factor(merged$affiliated)
merged$affiliated_appt <- as.factor(merged$affiliated)
@
...
<<returns_panel_c, dependson=c("return_data_prep_appt", "return_functions"), results='asis'>>=
