...
skill_summ <-
  tibble(Skill = c(skill_summ_labels, "Num. Skills"),
         skill_var = c(skill_vars, "num_skills"),
...
t_test_adams <- function(df, skill) {
...
skill_summ_sim <-
  tibble(Skill = skill_summ_labels,
         ...
         `Activist director` = t(skill_summ_raw %>% select(-num_skills))[, 1]) %>%
  inner_join(adams_data, by = "skill_var") %>%
  mutate(`Diff.` = `Activist director` - `Adams et al. (2018)`,
         `P-value` = unlist(lapply(skill_vars, function(x) t_test_adams(activist_director_skills, x)))) %>%
  select(-skill_var)
