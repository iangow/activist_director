...
Panel A presents data on directors.
Data on activist directors are restricted to those found on Equilar and relate to activist directors in the first year on their respective boards.
...
Values for \textit{Other boards} is the number of other boards served at the time of appointment, \textit{Board exp.} is the cumulative number of other boards served prior to appointment, \textit{Avg. tenure} is  the average number of years a director served on those other boards, and \textit{Total tenure} is the total number of years a director served in other boards, respectively.
