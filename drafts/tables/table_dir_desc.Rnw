...
Panel A presents data on directors matched to boardex. $N$ refers to the number of observations.
\textit{Age} refers to mean age of directors in each category.
Values for \textit{Female}, \textit{Comm.}, \textit{Comp comm.}, \textit{Audit comm.}, \textit{Fin. expert} are means of indicator variables for
being female, being a member of at least one board committee, being a member of the compensation committee,
being a member of the audit committee, and being designated a financial expert of the audit committee, respectively.
...
Panel B presents data on all activist directors (i.e., no requirement for boardex match).
