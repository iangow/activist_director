...
skill_vars <-
    activist_director_skills %>%
    select(academic:technology) %>% # num_skills
    colnames()
...
skill_summ <-
  tibble(Skill = c(skill_summ_labels, "Num. Skills"),
  ...
  mutate(`Diff.` = Unaffiliated - Affiliated,
         `P-value` = unlist(lapply(c(skill_vars, "num_skills"),
                                   function(x) t_test_fun(activist_director_skills, x))))

# Simulation Study
skill_probs <- c(0.075, 0.253, 0.082, 0.023, 0.340, 0.201, 0.089,
                 0.291, 0.287, 0.049, 0.383, 0.088, 0.113, 0.130,
                 0.234, 0.060, 0.014, 0.199, 0.016, 0.140)

set_names <- function(df) {
  colnames(df) <- skill_vars
  df
}

sim_director_skills_0 <-
  bind_cols(lapply(skill_probs, function(x) runif(29209) <= x)) %>%
  set_names() %>%
  mutate(affiliated = FALSE,
         unaffiliated = FALSE,
         activist_director = FALSE)

sim_director_skills <-
  activist_director_skills %>%
  collect() %>%
  mutate(affiliated = !independent,
         unaffiliated = independent,
         activist_director = !is.na(independent)) %>%
  bind_rows(sim_director_skills_0)

skill_summ_sim_raw <-
    sim_director_skills %>%
    filter(activist_director) %>%
    select(academic:technology) %>%
...
t_test_fun <- function(df, var) {
  formula <- as.formula(paste(var, "~ activist_director"))
  t.test(formula, data = df)[3]
}

skill_summ_sim <-
  tibble(Skill = skill_summ_labels,
         `Activist director` = t(skill_summ_sim_raw)[, 1],
          `Adams et al. (2018)` = t(skill_summ_adams)[, 1]) %>%
  mutate(`Diff.` = `Activist director` - `Adams et al. (2018)`,
         `P-value` = unlist(lapply(skill_vars, function(x) t_test_fun(sim_director_skills, x))))
rs <- dbDisconnect(pg)
@
