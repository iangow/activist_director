...
Values for \textit{Other boards}, \textit{Board exp.}, \textit{Avg. tenure}, and \textit{Total tenure} refer to means of the number of other boards served at the time, the number of boards served prior to appointment, the number of years a director served in other boards on average, and the total number of years a director served in other boards.
Panel A includes all directors, with \textit{Activist directors} relating to activist directors in the first year on their respective boards.
