...
activist_director_skills <- tbl(pg, "activist_director_skills") %>% collect()
...
board_experience <- equilar_final %>%
...
board_exp_by_period <-
    equilar_final %>%
        ...
        summarise(board_exp=count(date_start)) %>%
...
tenure_exp_by_period <-
    equilar_final %>%
        ...
        filter(date_start < period, company_id.x!=company_id.y) %>%
        mutate(company_id = company_id.x,
               tenure = pmax(0, pmin(period, company_director_max_term, na.rm=TRUE)-date_start, na.rm=TRUE)) %>%
        select(-company_id.x, -company_id.y) %>%
        distinct() %>%
        group_by(executive_id, company_id, period) %>%
...
board_experience <- tenure_exp_by_period %>%
                      group_by(executive_id, company_id) %>%
                      mutate(period=min(period)) %>%
                      inner_join(tenure_exp_by_period) %>%
                      select(-period) %>% collect()

all.dir.data <- all.dir.data %>% left_join(ind_exp, by=c("executive_id","company_id")) %>% left_join(tenure_exp_by_period) %>% distinct()
new.dir.data <- new.dir.data %>% left_join(ind_exp, by=c("executive_id","company_id")) %>% left_join(tenure_exp_by_period) %>% distinct()
act.dir.data <- act.dir.data %>% left_join(ind_exp, by=c("executive_id","company_id")) %>% left_join(board_experience) %>% distinct()

tab.1 <- getTab("all_directors", all.dir.data)
# New director sample - table
tab.2 <- getTab("category", new.dir.data) %>%
    mutate(Category = as.character(Category))
tab.3 <- getTab("activist_director.x", act.dir.data)
tab.4 <-
    getTab("affiliated_director.x", act.dir.data) %>%
...
tab2.1 <- getTab("all_directors", all.dir.data)
# New director sample - table
tab2.2 <- getTab("category", new.dir.data) %>%
    mutate(Category = as.character(Category))
tab2.3 <- getTab("activist_director.x", act.dir.data)
tab2.4 <-
    getTab("affiliated_director.x", act.dir.data) %>%
    arrange(desc(Category))
tab2 <- bind_rows(tab2.1, tab2.2, tab2.3, tab2.4)
...
rownames(skill_summ) <- c( "Academic", "Company business", "Compensation", "Entrepreneurial",
                               "Finance and Accounting", "Governance", "Government policy", "International",
                               "Leadership", "Legal", "Management", "Manufacturing", "Marketing",
                               "Outside board", "Outside executive", "Risk management",
                               "Scientific", "Strategic planning", "Sustainability", "Technology", "Num. Skills")
colnames(skill_summ) <- c("Activist director", "Affiliated", "Unaffiliated", "Diff.", "P-value")
...
rownames(skill_summ_sim) <- c( "Academic", "Company business", "Compensation", "Entrepreneurial",
                               "Finance and Accounting", "Governance", "Government policy", "International",
                               "Leadership", "Legal", "Management", "Manufacturing", "Marketing",
                               "Outside board", "Outside executive", "Risk management",
                               "Scientific", "Strategic planning", "Sustainability", "Technology")
colnames(skill_summ_sim) <- c("Activist director", "Adams 2018", "Diff.", "P-value")
@
...
@
\\
<<dir_desc_panel_A2, dependson="dir_desc_table", results='asis'>>=
# Note that display has to cover rownames as well (I set this to "s")
...
{\small
This table presents descriptive statistics for directors.
Panel C presents data on activist director skillsets with activist directors in Column 1 and the simulated sample of directors from Adams et al.(2018) in Column 2. The difference between activist director sample and Adams et al.(2018) sample is reported in Column 3 with its p-value reported in Column 4.
\newline}
% \end{flushleft}
\end{minipage}
\begin{tabularx}{\linewidth}{l*{5}{Y}}
\multicolumn{5}{l}{Panel C: Activist director skillsets: Activist directors}
% \hline
\\
<<dir_desc_panel_C, dependson="dir_desc_table", results='asis'>>=
# Note that display has to cover rownames as well (I set this to "s")
print(xtable(skill_summ_sim, display=c("s", "f", "f", "f", "f")),
...
{\small
This table presents descriptive statistics for directors.
Panel D presents data on activist director skillsets with affiliated directors in Column 2 and unaffiliated directors in Column 3. The difference between affiliated and unaffiliated directors is reported in Column 4 with its p-value reported in Column 5.
\newline}
% \end{flushleft}
\end{minipage}
\begin{tabularx}{\linewidth}{l*{6}{Y}}
...
% \hline
\\
<<dir_desc_panel_D, dependson="dir_desc_table", results='asis'>>=
# Note that display has to cover rownames as well (I set this to "s")
print(xtable(skill_summ, display=c("s", "f", "f", "f", "f", "f")),
      include.rownames=TRUE, only.contents=TRUE,
      format.args = list(big.mark = ","))
@
\end{tabularx}
