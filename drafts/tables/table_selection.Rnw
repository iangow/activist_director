...
prob_activist_director_skills <-
    as.data.frame(activist_demands) %>%
    left_join(as.data.frame(activist_director_skills)) %>%
    unique() %>%
    as_tibble() %>%
    collect()
...
%  \end{flushleft}
  \end{minipage}
    \begin{tabularx}{5in}{l*{3}{Y}}
     \multicolumn{3}{l}{Panel C: Activism, activist demands, and activist directors} \\
