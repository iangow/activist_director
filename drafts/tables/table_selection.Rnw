...
<<selection_data, cache=TRUE, results='asis'>>=
...
dissidents <-
    activism_events %>%
    mutate(dissident = unnest(dissidents)) %>%
    select(dissident_group, dissident)

big_dissidents <-
    activism_events %>%
    inner_join(dissidents) %>%
    group_by(dissident) %>%
    summarize(activism = sum(as.integer(activism), na.rm = TRUE),
           activist_demand = sum(as.integer(activist_demand), na.rm = TRUE),
           activist_director = sum(as.integer(activist_director), na.rm = TRUE)) %>%
    mutate(activist_demand_over_activism =
               if_else(activism > 0, activist_demand*1.0/activism, 0),
           demanding_activist = activism > 2 & activist_demand*1.0/activism > 0.50)
    # May need coalesce for demanding_activist

dissident_names <-
    dissidents %>%
    inner_join(big_dissidents, by = "dissident") %>%
    group_by(dissident_group) %>%
    summarize(demanding_activist = bool_or(demanding_activist))
...
prob_activist_director <-
    ...
    mutate(category = coalesce(category, "_none"),
           ...
           on_boardex = !is.na(permno)) %>%
    left_join(dissident_names, by = "dissident_group") %>%
...
rhs.1 <- "analyst inst size_return mv btm leverage dividend roa sale_growth
            num_directors outside_percent age tenure staggered_board"
