...
@
  ...
  {\small
This table presents results from logit regressions where the dependent variables are indicators for activist demanding board seats (\textit{Board demand}, Column 1), activists getting board seats (\textit{Activist director}, Column 2), activist directors not affiliated with activist (\textit{Unaffiliated director}, Column 3), and activist-affiliated directors getting board seats (\textit{Affiliated director}, Column 4).
...
\newpage
  ...
  \begin{table}[htb]
    \caption{Activist demands and target selection}
        ...
        \begin{minipage}[t]{\columnwidth}
  {\small
Panel A describes the types of demand activists have made as classified by FactSet SharkWatch.
Description is how SharkWatch describes each type of demand.
...
\newpage
  ...
  \begin{table}[htb]
    \caption{Activist demands and target selection}
        \begin{minipage}[t]{\columnwidth}
  {\small
Panel B presents results from logit regressions where the dependent variables are indicators for activist demanding board seats (\textit{Board demand}, Column 1), activists getting board seats (\textit{Activist director}, Column 2), and activist-affiliated and unaffiliated directors getting board seats (\textit{Affiliated director}, Column 3, \textit{Unaffiliated director}, Column 4), conditional on the firm being targeted by activists.
The control variables from Section \ref{sec:other_data} are included in all analyses.
...
\newpage
  ...
  \begin{table}[htb]
    \caption{Activist outcomes by demand}
        ...
        \begin{minipage}[t]{\columnwidth}
  {\small
Panel A presents success rates by type of demand as determined by FactSet SharkWatch.
Multiple demands can be made for each activism event.
Success rate (\%) reports the probability of activist demand being met by the boards successfully.
  \newline}
%  \end{flushleft}
  \end{minipage}
