...
\newpage
  ...
  \begin{table}[htb]
    \caption{Activist directors and activist demands}
        \begin{minipage}[t]{\columnwidth}
  {\small
Panel C presents logit regression where the dependent variables are indicators for activist demanding board seats (\textit{Board demand}, Column 1), activists getting board seats (\textit{Activist director}, Column 2), and activist-affiliated directors getting board seats (\textit{Affiliated director}, Column 3), conditional on the firm being targeted by activists. In Column 4, the sample only includes the firms with an activist director(s). The control variables from Panel B are included in all analyses.
  \newline}
%  \end{flushleft}
  \end{minipage}
    \begin{tabularx}{5in}{l*{3}{Y}}
