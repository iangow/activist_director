...
dirvoteresults  <- tbl(pg, "dirvoteresults")
...
{\small
This table presents the results of regressions where the dependent variable is voting support for directors.
...
The sample excludes contested elections, where vote support will be split between incumbents and challengers.
