...
get_reg_data <- function(vote_number) {
    ...
    ad_data <-
        ...
        mutate(appointment_date = pmin(meetingdate, appointment_date),
               start_date = appointment_date + make_difftime(day = (vote_number - 1) * 365),
               end_date   = appointment_date + make_difftime(day = vote_number * 365)) %>%
        filter(meetingdate >= start_date, meetingdate < end_date) %>%
        select(-campaign_id, -appointment_date, -vote_num,
               -start_date, -end_date)

    other_dirs <-
        director_votes %>%
        setdiff(ad_data %>% select(-independent)) %>%
...
Sample includes firm-years with and without activism, but excludes contested elections.
...
stargazer(fm_t0, fm_t0_iss,
          ...
          dep.var.labels = c("$\\textit{Support}_{t}$", "$\\textit{Support}_{t+1}$",
                             "$\\textit{Support}_{t+2}$"),
          covariate.labels = c("Activist director, independent",
