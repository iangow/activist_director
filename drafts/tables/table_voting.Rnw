...
{\small
This table presents regression results where the dependent variables are voting support for directors, activist directors (\textit{Activist director}) and affiliated directors (\textit{Affiliated director}), respectively.
Periods $t$, $t+1$, and $t+2$ refer to the first, second, and third elections for a director upon or after appointment.
Sample excludes contested elections, as vote support will be split between incumbent and challenger.
