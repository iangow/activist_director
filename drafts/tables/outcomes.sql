...
compustat_w_lags AS (
    SELECT gvkey, datadate, at, che, dvc, prstkc, pstkrv, oibdp,
        ...
        sum(ni) OVER p3 AS ni_cum_p3,
        sum(ib) OVER p3 AS ib_cum_p3,
        sum(xrd) OVER p3 AS xrd_cum_p3,
        sum(xad) OVER p3 AS xad_cum_p3,
        sum(dp) OVER p3 AS dp_cum_p3,
...
outcomes AS (
    SELECT a.gvkey, a.datadate,
        ...
        CASE WHEN at_p3 > 0 THEN che_p3/at_p3 END AS cash_p3,
        CASE WHEN oibdp > 0 THEN (dvc + prstkc - pstkrv)/oibdp END AS payout_cum,
        CASE WHEN oibdp_cum_p3 > 0
            THEN (dvc_cum_p3 + prstkc_cum_p3 - pstkrv_cum_p3)/oibdp_cum_p3 END AS payout_cum_p3,
        ...
        END AS leverage_p3,
        CASE WHEN at_m1 > 0 THEN xrd/at_m1 END AS rnd_cum,
        CASE WHEN at > 0 THEN (xrd_cum_p3)/at END AS rnd_cum_p3,
        CASE WHEN at_m1 > 0 THEN xad/at_m1 END AS adv_cum,
        CASE WHEN at > 0 THEN (xad_cum_p3)/at END AS adv_cum_p3,
        CASE WHEN at_m1 > 0 THEN capx/at_m1 END AS capex_cum,
        CASE WHEN at > 0 THEN (capx_cum_p3)/at END AS capex_cum_p3
    FROM compustat_w_lags AS a
    LEFT JOIN comp.names AS b
    ON a.gvkey=b.gvkey AND extract(year FROM a.datadate) BETWEEN year1 AND year2)

SELECT DISTINCT *
FROM activist_director.financials AS a
