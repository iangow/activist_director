...
activist_directors <- dbGetQuery(pg, "
    WITH raw_data AS (
        SELECT DISTINCT activist_affiliate,
       ...
       WHERE activist_affiliate IS NOT NULL)
...
names(activist_directors) <- c("Category", "N", "Left board", "Tenure",
                 "Still active", "Tenure")
activist_directors[,1] <- c("Affiliated directors","Unaffiliated directors")
...
act.dir.data <- dbGetQuery(pg, "
    SELECT 'Activist directors'::text AS activist_director,
      ...
      audit_committee_financial_expert
    FROM activist_director.equilar_w_activism
    WHERE age IS NOT NULL AND activist_director")

# New Director Sample
new.dir.data <- dbGetQuery(pg, "
  SELECT *
  FROM activist_director.equilar_w_activism
  WHERE age IS NOT NULL AND director_first_year")
